...
What it proves before any commit:
  ...
  D. the patched nightly on the real book matches an independent closed-form
     recomputation (registered formula, p_eff buckets, operated basis) and
     fires no guard;
...
BOT = os.path.join(HERE, "btc5m_bot_patched.py")
...
def qhat_new(lo):
    xs = [m for m in live_settled
          if (mod.Bot._p_eff_from_cost(mod.Bot._m_opcost(m)) < mod.IMP_BUCKET_LO) == lo]
...
check(bot.imp["qlo"] == qlo_new and bot.imp["qhi"] == qhi_new,
      "D: module nightly == independent closed-form (registered formula, p_eff buckets, operated basis)",
