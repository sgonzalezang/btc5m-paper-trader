...
#    (a) bucket boundary: was cost<0.50 (== p_eff<~0.4825); now p_eff<0.50
#        with p_eff = ask + slip exactly as §4.1/§4.2 define it
#        (IMP_BUCKET_LO). Measurement rows store cost; the boundary is
#        applied via the exact inverse _p_eff_from_cost.
...
IMP_BUCKET_LO = 0.50     # P2/R8: bucket boundary on p_eff = ask + slip (§4.2); was cost<0.50
def default_impulse():
...
class Bot:
    ...
    def _impulse_stake(self, p):
        ...
        cost = p + fee(p) per share; qhat bucketed by p_eff = p itself
        (< 0.50 / >= 0.50 — P2/R8 conformance to FINAL-DESIGN §4.2; the old
        cost<0.50 boundary mis-bucketed p_eff in [0.4825, 0.50) as hi);
        ...
        qh = imp["qlo"] if p < IMP_BUCKET_LO else imp["qhi"]
    ...
    def _impulse_nightly(self, ns):
        ...
        def qhat(bucket_lo):
            xs = [m for m in live if (self._p_eff_from_cost(self._m_opcost(m)) < IMP_BUCKET_LO) == bucket_lo]
...
def selftest():
    ...
    ba = mkimp(ask=0.52, bid=0.51)                           # first poll RICH: 54c fill -> f_nonpos at seeds
    av1 = ba.evaluate(now, "impulse_v2")
    ra = ba.imp["measure"][-1]
    ok(av1["stakeUsd"] is None and ra["skip"] == "f_nonpos" and ra.get("final") is False
    ...
    b21 = Bot({}, default_state(A))                          # p_eff=0.49 rows (cost .5075): LO bucket per §4.2
    b21.imp["measure"] = [dict(t0=ns - 3600 - i * 300, side="up", cost=0.5075, win=1, sized=True, skip=None)
                          for i in range(10)]
    b21._impulse_nightly(ns)
    ok(abs(b21.imp["qlo"] - round(110 / 210, 4)) < 1e-9 and b21.imp["qhi"] == 0.5,
       "P2: bucket boundary is p_eff<0.50 — cost .5075 (p_eff .49) rows feed the LO bucket",
       f"qlo={b21.imp['qlo']} (want {round(110/210,4)}) qhi={b21.imp['qhi']}")
    ...
    b22 = mkimp(ask=0.48, bid=0.47)                          # p_fill=0.49: LO bucket sizes at learned qlo
    b22.imp["qlo"], b22.imp["qhi"] = 0.53, 0.50
    v22 = b22.evaluate(now, "impulse_v2")
    ok(v22["enter"] and v22["stakeUsd"] and v22["stakeUsd"] >= 1,
       "P2: sizer buckets by p_eff — a 49c effective fill uses qlo (old cost-boundary sent it to qhi)",
       f"stake={v22['stakeUsd']}")
    b23 = mkimp(ask=0.49, bid=0.48)                          # p_fill=0.50 exactly: HI bucket per §4.2 [0.50, 0.53]
    b23.imp["qlo"], b23.imp["qhi"] = 0.56, 0.50
    v23 = b23.evaluate(now, "impulse_v2")
    ok(not v23["enter"] and b23.imp["measure"][-1]["skip"] == "f_nonpos",
       "P2: p_eff=0.50 exactly lands in the HI bucket (boundary is strict <)",
       f"skip={b23.imp['measure'][-1]['skip']}")
...
def main():
    ...
    bot = Bot({"publishEvery": args.publish_every, "sigEngines": sig_engines, "sigFile": args.signal_file,
               ...
               "seedPath": os.path.join(os.path.dirname(os.path.abspath(__file__)), "impulse_guard_seed.json")}, st)
